...
/**
  * @brief    call this function to register module response message
  *           handler in system_startup()
  */
void mesg_ack_register(uint16_t cmd_id);
...
void           uart_config(uint8_t id, uint32_t baudrate);
void        uart_send_buff(uint8_t id,uint8_t* buf, uint16_t len);
